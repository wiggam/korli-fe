...
export const InputBar = ({
  ...
  const sendAudio = async () => {
    ...
    try {
      ...
      setRecordingError(err instanceof Error ? err.message : 'Unable to send audio.');
    }
  };
      ...
      <div className="flex flex-col gap-2 rounded-2xl bg-[#0f1524] p-3 sm:flex-row sm:items-end">
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={sendText}
            disabled={!hasSession || disabled || !text.trim() || recordingState !== 'idle'}
            className="inline-flex h-11 items-center justify-center rounded-full bg-blue-500 px-6 text-sm font-semibold text-white transition hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <SendIcon />
          </button>
        </div>
